...
JUSTIFICATION: [Your explanation in ≤60 words]"""
        ...
        try:
            if 'gpt' in judge_model.lower():
                ...
                raise RuntimeError(
                    ...
                    "Use a model name containing 'gpt', 'claude', 'gemini', or 'gemma' "
                    "and configure the matching API key."
                )
    ...
    def _get_openai_critique(self, model: str, prompt: str, temperature: float) -> str:
        ...
        if 'openai' not in self.clients:
            raise RuntimeError("OpenAI client not configured. Set OPENAI_API_KEY before running.")
    ...
    def _get_anthropic_critique(self, model: str, prompt: str, temperature: float) -> str:
        ...
        if 'anthropic' not in self.clients:
            raise RuntimeError("Anthropic client not configured. Set ANTHROPIC_API_KEY before running.")
    ...
    def _get_google_critique(self, model: str, prompt: str, temperature: float) -> str:
        ...
        if 'google' not in self.clients:
            raise RuntimeError("Google client not configured. Set GOOGLE_API_KEY before running.")
...
def main():
    ...
    # Initialize judge (requires OPENAI_API_KEY / ANTHROPIC_API_KEY / GOOGLE_API_KEY in env)
    judge = ModelJudge()
